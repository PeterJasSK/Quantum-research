...
> **STATUS: EXECUTED (2026-08-17) — positive result, hypothesis confirmed.** Built as the
> teleport fork in `QuantumLife/code/research_qtree_teleport.py` + `research_qtree_swaplr.py`;
> results in `QuantumLife/research/conclusion_teleportation_longrange.md` and the paper
> `research/quantum-tree.tex`. Headline: a teleported long-range CNOT (Bell pair + 2
> mid-circuit measures + feed-forward) bonds two qubits 12 apart, **never neighbours**, at
> **constant depth 9** and leaves a stable, single-sign, crosstalk-immune correlation
> (`c(d) ≈ −0.065`, best −0.116/−0.139, all seeds/submissions negative). An **exact noiseless
> simulation** (`code/sim_ideal_sign.py`) then proved the teleport circuit is logically a CNOT
> and the ideal bond sign is **negative** — teleport reproduces it on hardware, while the
> depth-31 SWAP ladder decoheres it to a wrong-signed floor. **Verdict: teleportation is the
> faithful, better long-range gate; the genome carries a real correlated quantum bond.** The
> crosstalk-vs-entanglement confound the parent study flagged is closed. Follow-ons spun out
> as `qh-10` (routing benchmark), `qh-11` (entanglement-reach metric), `qh-12` (SWAP
> depth-inversion). Remaining: distance sweep + second backend (scale, not the claim).
