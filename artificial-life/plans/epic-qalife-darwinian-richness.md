...
| P2 ✅ **DONE** (2026-09-11) | Scale ceiling (the "how big" axis) | `research/P2_CONCLUSION.html` results page + `research_runs/3.*` damping runs | P1 | **DONE.** Two axes on live `ibm_kingston`: unitary genealogy **W\*=24=48 qubits** (k=2σ, ~6× paper; W=32 dead) proven; faithful **amplitude-damping death channel** fixed 0.35→0.55 at W=2, certified k=3σ through **W=4** (W=2 +0.545, W=3 +0.782, W=4 +0.648), collapses to +0.001 by W=8 → death-channel W\* ∈ [4,8). Confound: ideal-vs-hardware separated; separable null ≤2e-6. **Gate:** unitary scale exhausted; death-channel axis is the live frontier — pin collapse edge (**W=5, W=6 + mitigation**, open) before broad biology. Results: `research/P2_CONCLUSION.html`. |
