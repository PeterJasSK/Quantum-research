...
Vendored near-verbatim from
`../TargetedDosColisionsAndRNGAngle/testbed/salt/qrng_client.py` (structural
twin) -- same retry policy, same error envelope, same `X-API-Key` header. No
new HTTP behaviour to design or review. Error envelope verified against
`qrng-eaas/api/qeaas/errors.py`: `{"error": "<slug>"}`, with a `Retry-After`
header on 429.
"""
...
# misconfiguration and fails loudly on the first attempt.
_MAX_ATTEMPTS = 3
